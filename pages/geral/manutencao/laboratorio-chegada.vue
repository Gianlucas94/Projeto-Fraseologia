<template>
  <div>
    ...
    <div class="uk-grid uk-grid-medium uk-child-width-expand@s uk-margin">
      <div>
        <div class="uk-margin">
          <label for="data-retorno" class="uk-form-label required">Retorno de laboratório</label>
          <date-pick v-model="dataRetorno" @input="updateDetalhes" :displayFormat="'DD/MM/YYYY'" :prevMonthCaption="'Anterior'" :nextMonthCaption="'Próximo'" :setTimeCaption="'Hora:'" :weekdays="['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab', 'Dom']" :months="['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembero', 'Outubro', 'Novembero', 'Dezembro']"> </date-pick>
        </div>
      </div>
      <div>
        <div class="uk-margin">
...
export default {
  ...
  data() {
    return {
      resumo: 'Laboratório - Chegada de equipamento ou peça',
  ...
  mounted() {
    ...
    this.dataRetorno = Fraseologia.today();
    this.data = Fraseologia.today();
    this.hora = Fraseologia.now();
    this.dataHtml = Fraseologia.todayHtml(this.data);
    this.dataRetornoHtml = Fraseologia.todayHtml(this.dataRetorno);

    this.$store.commit('updateResumo', this.resumo);
    this.$store.commit('updateStatus', this.status);

    this.detalhes = `Equipamento retornou de laboratório externo: ${this.dataRetornoHtml}
Estamos realizando testes em bancada.
Previsão de término da atividade: ${this.dataHtml} às ${this.hora}.`;
